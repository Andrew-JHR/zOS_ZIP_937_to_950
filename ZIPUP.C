 ...
 */
{
  ...
  if (translate_eol == 0) {
    ...
    if (aflag == ASCII)
    {
       ...
       /* update the following mark-out line for dbcs conversion*/
       /* else *buf++ = (char)(c == '\n' ? LF : ascii[(uch)c]); */
          else if ( *buf == '\n' )
          {
            *buf++ = LF ;
          ...
          {
            *buf++ = 0x20;      /*next byte*/
            odd = 'N';          /*set off the ind.*/
          }
          else if ( *buf == 0x0e )
          {
            *buf++ = 0x20;      /*replace with space*/
            odd = 'Y';          /*set on the ind.*/
          }
          else if ( odd == 'Y' )
          {
            unsigned int i;    /* temp. for cal. xlat. tab. offsets*/
            if ( size >= 2)    /* make sure at least 2 bytes left*/
             {
               i=(*buf-0x40)*(0xfe-0x40)*2+(*(buf+1)-0x40)*2;
               *buf++ = dbcsh2p[i++];  /*higher byte*/
               *buf++ = dbcsh2p[i];    /*lower byte*/
               size--;     /*decre. 1 only, the other done on while*/
             }
            else  /* size < 2, cannot be xlated now */
             {
               /*printf("DBCS Byte Split\n");warning*/
               len--;              /*modify the total len*/
               fseek(ifile,-1,SEEK_CUR); /*back file ptr 1 byte*/
             }    /* end of size < 2 */
          }     /*end of odd=yes*/
          else if ( *buf <  0x40 )
          {
            *buf++ = 0x20;      /*force the unprintable into spaces*/
          }     /*end of < 0x40 */
          else  /*for other single-byte codes*/
          {
             *buf++ = (char)ascii[(uch)(*buf)]; /*ascii*/
          }     /*end of single byte xlation*/
